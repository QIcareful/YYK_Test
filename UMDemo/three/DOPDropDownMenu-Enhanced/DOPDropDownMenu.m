 ...
 *  add
 */
@interface DOPDropDownMenu ()<dropdownDelegate>{
    struct {
    ...
    ConditionDoubleTableView *tableViewS;
    NSArray *_titleArray;
    NSArray *_leftArray;
    NSArray *_rightArray;
...
@property (nonatomic, strong) UIView *chooseView;//在视图中间插一层
@property (nonatomic, strong) DropdownMenu *dropdown;//第三层
...
@property (nonatomic, strong) UISegmentedControl*segmentedControl;//分类两个切换
//测试
@property (nonatomic, strong) UIButton *btn;
@property (nonatomic, strong) UIButton *btt;
...
#define kTableViewCellHeight 43 //展开行的高度
#define kTableViewHeight 300 //展开页面的总高度 当视图小于展开行的高度时候
#define kButtomImageViewHeight 0//下面的点击视图高度
#define kChoose 650//占位的高度
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    NSLog(@"x=%fy=%f",origin.x,origin.y);
    if (self) {
        _origin = origin;
        ...
        UIView *chooseView = [[UIView alloc] initWithFrame:CGRectMake(origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, kChoose)];
//        chooseView.backgroundColor = [UIColor blueColor];
        _chooseView = chooseView;
        //lefttableView init
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, self.frame.origin.y + self.frame.size.height + kChoose, self.frame.size.width/2, 0) style:UITableViewStylePlain];
        ...
        //righttableView init
        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x + self.frame.size.width/2, self.frame.origin.y + self.frame.size.height + kChoose, self.frame.size.width/2, 0) style:UITableViewStylePlain];
        ...
        _buttomImageView = [[UIImageView alloc]initWithFrame:CGRectMake(origin.x, self.frame.origin.y + self.frame.size.height+kChoose, self.frame.size.width, kButtomImageViewHeight)];
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(origin.x, origin.y + kChoose, screenSize.width, screenSize.height)];
        ...
        //add bottom shadow
        UIView *bottomShadow = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.size.height-0.5 + kChoose, screenSize.width, 0.5)];
        ...
        [btn setTitle:@"woo" forState:UIControlStateNormal];
        btn.backgroundColor = [UIColor redColor];
        UIButton *btt = [UIButton new];
        [btt setTitle:@"wow" forState:UIControlStateNormal];
        ...
        _btt = btt;
        _label = label;
//       UISegmentedControl *segmentedControl =  [[UISegmentedControl alloc]initWithItems:[[NSArray alloc]initWithObjects:@"商区",@"地铁",nil]];
//        segmentedControl.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 44);
...
- (CALayer *)createBgLayerWithColor:(UIColor *)color andPosition:(CGPoint)position {
    ...
    layer.bounds = CGRectMake(0, 0+90, self.frame.size.width/self.numOfMenu, self.frame.size.height-1);
...
{
//    [_dropdown.view removeFromSuperview];
...
#warning mark ====================================
    if (show) {
        if (haveItems) {
            _chooseView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height + 44,self.frame.size.width , kChoose);
            _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height + kChoose, self.frame.size.width/2, 0);
            _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.frame.origin.y + self.frame.size.height + kChoose, self.frame.size.width/2, 0);
            _dropdown.view.frame = CGRectMake(self.origin.x, 0,self.frame.size.width , kChoose);
            [self.superview addSubview:_chooseView];
            [self.superview addSubview:_dropdown.view];
            [self.superview addSubview:_leftTableView];
            [self.superview addSubview:_rightTableView];
        } else {

            _chooseView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height + 44,self.frame.size.width , 0);
            ...
            _dropdown.view.frame = CGRectMake(self.origin.x, 0,self.frame.size.width , 0);
            _label.frame = CGRectMake(60, 120, 40, 25);
            _btt.frame = CGRectMake(20, 160, 40, 25);
            _btn.frame = CGRectMake(90, 190, 40, 25);
            [self.superview addSubview:_chooseView];
            [self.superview addSubview:_btn];
            [self.superview addSubview:_btt];
...
//            [self.superview addSubview:_dropdown.view];
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                _chooseView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height,self.frame.size.width , kChoose);
                _leftTableView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height + kChoose, self.frame.size.width/2, tableViewHeight);
                
                _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.frame.origin.y + self.frame.size.height + kChoose, self.frame.size.width/2, tableViewHeight);
            } else {
                _chooseView.frame = CGRectMake(self.origin.x, self.frame.origin.y + self.frame.size.height,self.frame.size.width , kChoose);
...
//        UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 44, 375, 60)];
...
- (void)testLeftArray {
    NSArray *One_leftArray = @[@"附近", @"热门商品", @"西湖区", @"上城区", @"下城区",@"滨江区"];
